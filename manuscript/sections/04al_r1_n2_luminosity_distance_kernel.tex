...
The remaining light-cone observable is evaluated with the gauge-invariant geometric luminosity-distance construction.\footnote{J. Yoo and F. Scaccabarozzi, JCAP 09 (2016) 046, arXiv:1606.08453.}
For positive spatial curvature the background transverse distance and lensing efficiency use $S_K(\chi)=\sin(\sqrt K\chi)/\sqrt K$, with the corresponding curved-FLRW line-of-sight kernel.\footnote{See also T. Pyne and M. Birkinshaw, MNRAS 348 (2004) 581 for the linear luminosity distance in curved perturbed FLRW spacetimes.}
...
\end{equation}
with $C_K(\chi)=\cos(\sqrt K\chi)$ and the redshift, radial and convergence distortions evaluated along the same R1 physical-$n=2$ metric response.
...
\begin{equation}
\boxed{
\mathcal F_{D_L}(z_\star)=(1.842472667e-01,\,-3.520884651e-03).
}
\end{equation}

The carried-forward SN calibration imposes one scalar constraint on this two-dimensional state:
\begin{equation}
\mathcal F_{D_L}(z_\star)\cdot X_{2,\rm anchor}=-1.897330117e-02\pm4.144653167e-03.
\end{equation}

The corresponding unit null direction is
\begin{equation}
n_{\rm cal}=(1.910607424e-02,\,9.998174623e-01),\qquad \mathcal F_{D_L}\cdot n_{\rm cal}=0.
\end{equation}
