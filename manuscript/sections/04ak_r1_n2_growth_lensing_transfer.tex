...
The $f\sigma_8$ object above is a state-space fractional response kernel, using the R1 baseline $dD/dN$ normalization.  It is not yet the final single-number directional prediction because one $D_L$ calibration row cannot select the complete initial-state vector.
