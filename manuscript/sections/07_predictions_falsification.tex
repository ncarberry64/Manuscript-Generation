...
These numbers are a carried-forward calibration input; the present repository does not independently rerun the Pantheon+ tomographic likelihood. They normalize only the selected physical \(n=2\) response,
\[
A_\mu
=
...
\]
so the extra scalar susceptibility falls as \(n^{-2}\). The model therefore predicts a strong separation between the first physical compact-space level and conventional clustering modes.
