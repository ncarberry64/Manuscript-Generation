...
\]
and do not independently determine \(S_2\), \(\mathcal K_{\rm SN}\), or \(C_2\). Their interpretation as a metric/light-cone response is additionally conditional on resolving the exceptional closed-FLRW \(n=1\) sector.
...
The bridge predicts selective \(n=2\) softening with no analogous \(n\ge3\) enhancement. A future source-normalized \(S^3\) \(n=2\) susceptibility comparable to or larger than the \(n=1\) susceptibility would reject the minimal harmonic-filter branch. This statement is made at the compact-space harmonic level; comparison to a measured sky quadrupole requires an explicit observer-shell projection.
